...
t (maximum sum of distances, integer >= 0), k (number of towns to visit, k >= 1) and ls (list of distances,
all distances are positive or null integers and this list has at least one element). The function returns the "best" sum
ie the biggest possible sum of k distances less than or equal to the given limit t, if that sum exists, or otherwise nil, null, None, Nothing,
...
def choose_best_sum(t, k, ls)
  ls.combination(k).collect { |distances| distances.inject(:+) }.reject { |total| total > t }.max
end

ts = [50, 55, 56, 57, 58]
ap choose_best_sum(163, 3, ts)
# 163

xs = [50]
ap choose_best_sum(163, 3, xs)
# nil

ys = [91, 74, 73, 85, 73, 81, 87]
ap choose_best_sum(230, 3, ys)
# 228
